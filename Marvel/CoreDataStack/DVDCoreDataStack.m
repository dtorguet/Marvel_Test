...
@import CoreData;//nueva forma de importar FrameWork de sistema desde iOS7
...
-(NSPersistentStoreCoordinator *) storeCoordinator{
    if (_storeCoordinator == nil) {
        ...
        // Estos dos diccionarios es para decirle en opciones que migre automaticamente el modelo
...
-(id) initWithModelName:(NSString *)aModelName
    ...
    if (self = [super init]) {
        self.modelURL = [[NSBundle mainBundle] URLForResource:aModelName
                                                withExtension:@"momd"];
        //esto de momd es una compilacion del xml a un fichero binario
        self.dbURL = aDBURL;
    }
...
-(void) saveWithErrorBlock: (void(^)(NSError *error))errorBlock{
    ...
    //esto es importante le pregunto si hay algo que guardar si no lo hay ejecuto error
